...
class Trade extends Model
{
    ...
    {
        $sql = "SELECT id,transaction,app_id,user_id,currency,amount,amount_usd,status,gateway,product_id,custom,ip,ext
FROM `transactions` WHERE transaction=:transaction LIMIT 1";
     ...
     * @return mixed
     */
    public function getTradeMore($transaction = '')
    {
        $sql = "SELECT
t.id,t.transaction,t.app_id,t.user_id,t.currency,t.amount,t.amount_usd,t.status,t.gateway,t.product_id,t.custom,t.ip,t.create_time,
m.trade_no,m.key_string,m.data
FROM `transactions` t, `trans_more` m
WHERE t.transaction=:transaction AND t.transaction=m.trans_id LIMIT 1";
        $bind = array('transaction' => $transaction);
        $query = DI::getDefault()->get('dbData')->query($sql, $bind);
        $query->setFetchMode(Db::FETCH_ASSOC);
        return $query->fetch();
...
LIMIT 1";
    ...
    public function createTrade($tradeData = [], $more = [])
    {
        ...
        if (isset($tradeData['product_id'])) {
            $sql = "SELECT id,price,currency,name,remark FROM `products` WHERE status=1 AND product_id=:product_id";
        ...
        $trade = array(
            ...
            "ext" => isset($tradeData['ext']) ? $tradeData['ext'] : 0,
            "create_time" => date('Y-m-d H:i:s')    // 不使用SQL自动插入时间，避免时区不统一
        );

        // 转美金
        if ($trade['status'] == 'paid') {
            $trade['amount_usd'] = $this->changeToUSD($trade['amount'], $trade['currency']);
        }
        ...
        $trade['product_name'] = isset($product['name']) ? $product['name'] : '';
    ...
    public function getAppConfig($app_id = 0)
    {
        ...
        $data = $query->fetch();
        if (!$data) {
            writeLog('no app config:' . $app_id, 'error' . date('Ym'));
        }
        return $data;
    }
...
WHERE status=1 AND app_id=:app_id AND gateway=:gateway AND price=:amount AND currency=:currency";
    ...
    {
        if (!$gateway) {
            $sql = "SELECT id,parent,`name`,remark,gateway,sub,currency,tips FROM `gateways` WHERE visible=1 AND app_id=:app_id AND parent = 0 ORDER BY sort DESC";
            $bind = array('app_id' => $app_id);
        }
        else {
            $sql = "SELECT id,parent,`name`,remark,gateway,sub,currency,tips FROM `gateways` WHERE visible=1 AND app_id=:app_id AND parent !=0 AND gateway=:gateway ORDER BY sort DESC";
    ...
    {
        $sql = "SELECT * FROM `transactions` WHERE transaction=:orderId";
    ...
    /**
     * ["card"=>"MC123456"] or ["startTime"=>1500000000,"endTime"=>1560000000];
     * @param array $argv
     * @return array
     */
    public function getMyCardCheckData($argv = [])
    {
        $result = [];
        if (!empty($argv['card'])) {
            $sql = "SELECT t.transaction,t.user_id,t.currency,t.amount,t.status,t.custom,t.complete_time,m.trade_no,m.data
                FROM `transactions` t, `trans_more` m
                WHERE t.gateway='mycard' AND t.status='complete' AND t.transaction=m.trans_id AND m.data LIKE '%{$argv['card']}%'";
            $query = DI::getDefault()->get('dbData')->query($sql);
            $query->setFetchMode(Db::FETCH_ASSOC);
            $data = $query->fetchAll();
        }
        elseif (!empty($argv['startTime']) && !empty($argv['endTime'])) {
            $sql = "SELECT t.transaction,t.user_id,t.currency,t.amount,t.status,t.custom,t.complete_time,m.trade_no,m.data
                FROM `transactions` t, `trans_more` m
                WHERE t.gateway='mycard' AND t.status='complete' AND t.transaction=m.trans_id AND t.complete_time>:startTime AND t.complete_time< :endTime";
            $bind = array(
                'startTime' => date("Y-m-d H:i:s", $argv['startTime']),
                'endTime'   => date("Y-m-d H:i:s", $argv['endTime'])
            );
            $query = DI::getDefault()->get('dbData')->query($sql, $bind);
            $query->setFetchMode(Db::FETCH_ASSOC);
            $data = $query->fetchAll();
        ...
        foreach ($data as $value) {
            parse_str($value['data'], $d);
            $result[] = [
                'type'                 => $d['PaymentType'],        // INGAME, COSTPOINT Or Something Else
                'transactionId'        => $value['transaction'],    // My Transaction Id
                'transactionReference' => $value['trade_no'],       // MyCard Transaction Id
                'card'                 => $d['MyCardTradeNo'],      // Card Number Or Something Else
                'amount'               => $value['amount'],         // Amount
                'currency'             => $value['currency'],       // Currency
                'account'              => $value['transaction'],    // User Id
                'time'                 => strtotime($value['complete_time']) // Timestamp
            ];
        }
        return $result;
    }
